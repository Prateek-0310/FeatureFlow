// src/webFeaturesLoader.ts
interface BaselineStatus {
    status: 'widely' | 'newly' | 'limited' | undefined;
    low_date?: string;
    high_date?: string;
}
...
let webFeaturesData: WebFeaturesData = {};
let cssPropertyMap: Map<string, FeatureData> = new Map();
let htmlElementMap: Map<string, FeatureData> = new Map();

export async function initializeWebFeatures(): Promise<void> {
    try {
...
function buildFeatureMaps(): void {
    // Clear previous maps
...
function extractCSSPropertyName(featureId: string): string | null {
    // Remove 'css-' prefix and convert to property name
    const name = featureId.replace('css-', '').replace(/-/g, '-');
    return name;
}

function extractHTMLElementName(featureId: string): string | null {
    // Remove 'html-' prefix
    const name = featureId.replace('html-', '');
    return name;
}

export function getCSSFeature(property: string): FeatureData | undefined {
    return cssPropertyMap.get(property.toLowerCase());
}

export function getHTMLFeature(element: string): FeatureData | undefined {
    return htmlElementMap.get(element.toLowerCase());
}

export function getAllFeatures(): WebFeaturesData {
...
function getWebFeaturesData(): WebFeaturesData {
    return {
